...
class Queue():
    ...
    def Dequeue(self):
        if self.outbox.is_empty(): # __stack of outbox is empty
            while self.inbox.is_empty() is False:
                item = self.inbox.Pop()
                self.outbox.Push(item) # Add item from inbox to 2 stacks of outbox
        self.outbox.Pop() # Remove the last item from outbox

    def Max(self):
        inmax = self.inbox.Max()
        outmax = self.outbox.Max()
